...
class PrintVisitor extends TreeVisitor {

  var level = 0

  override def visit(node: EnhancedCommonTree) = {
    val spaces = "  " * level
    if (node.getChildCount > 0) level += 1
    else if (node.getChildCount == 0) level -= 1
    println("[VISIT] %s %4d: %s".format(spaces, node.token.getType, node.token.getText))
  }
}
